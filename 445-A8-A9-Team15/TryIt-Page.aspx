...
<%-- CAPTCHA STARTS HERE--%>
    <div>
        <h1><b>Captcha User Control: Try-It Page</b></h1>
        <h3>The Captcha User Control is a user control that generates a captcha image with a random 6-character string.</h3>
        <h3>There is also a button to generate a new image.</h3>
        <p>Captcha User Control: Input of string, output of string</p>
        <br/>
        <captcha:CaptchaControl ID="MyCaptcha" runat="server"/>
        <br/>
        <asp:TextBox runat="server" ID="CaptchaGuessText"></asp:TextBox>
        <br/>
        <asp:Button runat="server" ID="CaptchaGuessBtn" Text="Submit" OnClick="CaptchaGuessBtn_Click"/>
        <br/>
        <asp:Label runat="server" ID="Output" Text="Captcha results will appear here"></asp:Label>
    </div> 
